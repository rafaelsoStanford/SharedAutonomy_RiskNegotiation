...
class Downsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.Conv1d(dim, dim, 3, 2, 1) # dim_in, dim_out, kernel_size, stride, padding -- Reduce signal size by half
...
class Upsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.ConvTranspose1d(dim, dim, 4, 2, 1)   # dim_in, dim_out, kernel_size, stride, padding -- Double signal size
...
class Conv1dBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, kernel_size, n_groups=8):
        ...
        self.block = nn.Sequential(
            nn.Conv1d(inp_channels, out_channels, kernel_size, padding=kernel_size // 2),   # Signal keeps same size, but channels change
            nn.GroupNorm(n_groups, out_channels), # GroupNorm normalizes over the channel dimension
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        cond_dim = dsed + global_cond_dim # Channels of dsed and global_cond_dim are concatenated
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            down_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(     # Residual block with Film
        ...
        final_conv = nn.Sequential(
            ...
            nn.Conv1d(start_dim, input_dim, 1), # Final output is the same size as input, e.g. action dim
        )
    ...
    def forward(self, 
        ...
        if global_cond is not None:
            global_feature = torch.cat([
                global_feature, global_cond # Concatenate global feature and global condition, Which is then added in Residual block
            ], axis=-1)
