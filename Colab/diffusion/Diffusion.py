...
from Diffusion_DDPM import *
from Unet_simple import *
...
class Diffusion(pl.LightningModule):
    def __init__(self, noise_steps=1500
                , lr=1e-4
        ...
        self.noise_predictor = UNet_simplified(
            cond_dim = self.global_cond_dim * T_obs
        )
    ...
    def sample(self, cond_features: torch.Tensor, cond: torch.Tensor):
        ...
        # Sample noise - (1, T_pred, diffusion_out_dim)
            ...
            nAction = nAction.unsqueeze(1) # (B, 1, T_pred, diffusion_out_dim)

            for k in reversed(range(1, self.noise_steps)):
                # predict noise
                t_step = torch.tensor(k, device= self.device)
                t_step = t_step[:, None]
    ...
    def onepass(self, batch, batch_idx, mode):
        ...
        # Modify actions by including a channel dimension
        noisy_actions = noisy_actions.unsqueeze(1) # (B, 1, T_pred, 7)
        ...
        predicted_noise = predicted_noise.squeeze(1) # (B, T_pred, 7)
